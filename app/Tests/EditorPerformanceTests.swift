    ...
    func testTypingInPlainProseAvoidsReparsing() {
        ...
        print(
            "[perf] prose keystroke on 10k lines: "
                + "\(String(format: "%.2f", samples[0] * 1000))ms fastest, "
                + "\(String(format: "%.2f", samples[samples.count / 2] * 1000))ms median, "
                + "\(String(format: "%.2f", samples[samples.count - 1] * 1000))ms worst, "
